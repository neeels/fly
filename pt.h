...
class Pt {
  public:
    ...
    static void wrap_coord(double &x, double c, double dist) {
      /* c is the zero reference, i.e. an x that was at c should be zero after
       * this. */
      double xx = x - c;

      /* dist is like a radius around c, so I really want to wrap at dist*2
       * around zero. To ease modulo calculations, shift -dist..dist to
       * 0..2*dist, wrap, then shift back to -dist..dist. */

      if (xx < -dist) {
        double d = dist * 2.;
        xx -= dist;
        xx -= d * (double)((int)( xx / d ) );
        xx += dist;
      }
      else
      if (xx > dist) {
    ...
    void wrap_cube(const Pt &center, double dist) {
      wrap_coord(x, center.x, dist);
      wrap_coord(y, center.y, dist);
      wrap_coord(z, center.z, dist);
    }
